...
Dark, almost monochrome, with one electric blue for emphasis. Few colors and no
in-between shades: if a new color seems necessary, the design is wrong first.
...
| `surface` | `#141619` | Cards, panels, inputs |
| `line` | `#22252A` | Borders and dividers |
...
- Borders: `1px` in `line`. Surfaces are separated by borders, not shadows.

## Tailwind v4 tokens

Paste into `src/app/globals.css`. Tailwind generates `bg-background`, `text-accent`,
`border-line`, `rounded-sm`, etc. from these.
